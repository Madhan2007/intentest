...
public class IdentityController {
    ...
    public ResponseEntity<ApiEnvelope<Map<String, Object>>> session(Authentication authentication,
        ...
        if (session.getCompanyId() != null && !session.getCompanyId().isBlank()) {
            user.put("c", session.getCompanyId());
        }
    ...
    private Map<String, Object> sessionPayload(LoginResult result) {
        ...
        if (result.companyId() != null && !result.companyId().isBlank()) {
            user.put("c", result.companyId());
        }
        Map<String, Object> data = new LinkedHashMap<>();
        data.put("user", user);
